...
const DealerManagement = () => {
    ...
    const archiveDealer = async (d: Dealer) => {
        if (!confirm(`Archive dealer "${d.name}"? Their cars remain listed.`)) return;
        const { error } = await supabase.from('dealers').update({ status: 'archived' }).eq('id', d.id);
        if (error) showToast(error.message, 'error');
        else { showToast('Dealer archived.'); fetchAll(); }
    };
                            ...
                            {filteredDealers.map(dealer => (
                                <div key={dealer.id} className="bg-white rounded-2xl border border-slate-100 p-5 shadow-[var(--shadow-card)] hover:shadow-md transition-shadow">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {dealer.status !== 'archived' && (
                                            <button
                                                onClick={() => archiveDealer(dealer)}
